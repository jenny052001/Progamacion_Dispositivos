...
class AudioUtiles(
    ...
    private fun grabaStop() {
        if (ContextCompat.checkSelfPermission(contexto, Manifest.permission.RECORD_AUDIO) !=
            ...
            grabando = if (!grabando) {
                mediaRecorderInit() // se inicializa el microfono
                iniciaGrabacion()
                true
            } else {
                detenerNota()
    ...
    private fun mediaRecorderInit() {
        if (audioFile.exists() && audioFile.isFile) { // ver si existe un archivo y se borra para hacer uno nuevo
            audioFile.delete()
        }
    ...
    private fun iniciaGrabacion() {
        try {
            mediaRecorder?.prepare() // se prepara el microfono
            mediaRecorder?.start() // empieza a grabar
            Toast.makeText(contexto,msgIniciaNotaAudio,Toast.LENGTH_LONG).show()
            btAccion.setImageResource(R.drawable.ic_stop) // se cambia el icono a STOP
            btPlay.isEnabled = false
            btDelete.isEnabled = false
    ...
    private fun playNota() { // para saber si el archivo existe y se pueda reproducir
        try {
